  ...
  async scrape(): Promise<ScrapedEvent[]> {
    ...
    try {
      ...
      for (const element of eventElements) {
        try {
          const title = await element.$eval('.event-title', el => el.textContent);
          const description = await element.$eval('.event-description', el => el.textContent).catch(() => '');
          const location = await element.$eval('.event-location', el => el.textContent).catch(() => '');
          const venue = await element.$eval('.event-venue', el => el.textContent).catch(() => location);
          const startDateStr = await element.$eval('.event-start-date', el => el.textContent).catch(() => '');
          const endDateStr = await element.$eval('.event-end-date', el => el.textContent).catch(() => startDateStr);
          const imageUrl = await element.$eval('.event-image', el => el.getAttribute('src')).catch(() => null);
          const priceStr = await element.$eval('.event-price', el => el.textContent).catch(() => '');
          const eventUrl = await element.$eval('a', el => el.getAttribute('href')).catch(() => null);
